...
public record DatabaseSettings(String url, String username, String password) {
    ...
    public static DatabaseSettings fromEnvironment() {
        String password = firstNonBlank("ACCOUNTING_DB_PASSWORD", "DB_PASSWORD");
        if (password == null || password.isBlank()) {
            password = new SecretsManagerPasswordResolver(new ObjectMapper()).resolve(
                    System.getenv("ACCOUNTING_DB_PASSWORD_SECRET_ARN"),
                    System.getenv("ACCOUNTING_DB_PASSWORD_SECRET_JSON_KEY"));
        }
        return new DatabaseSettings(
                firstNonBlank("ACCOUNTING_DB_URL", "DB_URL"),
                firstNonBlank("ACCOUNTING_DB_USERNAME", "DB_USERNAME"),
                password);
    }
