...
- RN-084: Una resolucion o emision electronica `ELECTRONIC_INVOICE`, `ELECTRONIC_POS`, notas credito/debito o nota de ajuste POS solo puede habilitarse cuando la empresa tenga configuracion DIAN propia en modo `REAL`, activa, probada exitosamente y con certificado configurado. La ausencia de esa configuracion no habilita por si sola `NON_FISCAL_SALE`: esta solo se permite con clasificacion vigente `NOT_OBLIGATED_VERIFIED` y no imprime CUFE/CUDE ni QR DIAN.
...
- RF-201: Debe existir un modulo `Configuracion contable` dentro de `Configuracion` para inicializar plantilla basica PUC, consultar cuentas y consultar reglas contables por empresa.
...
- RF-279: La conexion real DIAN de habilitacion debe consumir el servicio SOAP WCF `https://vpfe-hab.dian.gov.co/WcfDianCustomerServices.svc?wsdl` o su `singleWsdl`, usando operaciones DIAN oficiales como `SendTestSetAsync`, `GetStatusZip` y `GetStatus` segun el flujo de habilitacion.
- RF-280: El transporte real DIAN no puede ser un POST HTTP XML simple; debe construir envelope SOAP, headers requeridos, accion SOAP por operacion, payload ZIP/base64 cuando aplique y mapear respuestas SOAP DIAN a estados funcionales internos.
- RF-281: El modulo de Configuracion DIAN debe reemplazar la captura de certificado como texto por carga segura de archivo `.p12` o `.pfx`, con password de certificado como secreto de solo entrada, validacion de extension/tamano/tipo, extraccion de metadata no sensible y almacenamiento por referencia segura aislada por `company_id`. ROOT puede administrar el certificado de una empresa seleccionada, pero no existe certificado ROOT/global reutilizable para emitir por otras empresas.
- RF-282: El sistema debe permitir que empresas no obligadas o no preparadas para transmitir a DIAN cierren ventas internas no fiscales, liquidando impuestos segun la politica contable definida por la empresa, sin generar resolucion electronica, CUFE, CUDE, QR DIAN ni envio al conector DIAN.
- RF-283: El modulo ROOT de licencias debe ofrecer planes comerciales `POS y facturacion`, `Completo` y `Personalizado`, eliminando `Basico`; `Completo` selecciona todos los modulos licenciables, `POS y facturacion` selecciona empresa/configuracion, inventario, ventas/facturacion electronica, reportes, clientes/proveedores, contabilidad y usuarios/roles, y `Personalizado` permite seleccionar manualmente los modulos contratados.
...
## Requisitos fase modulo de contadores, reglas fiscales y notificaciones

- RF-284: La plataforma debe incluir un modulo `Contadores` para usuarios contadores externos que actuen como nicho comercial y asesores fiscales/contables de empresas vinculadas.
- RF-285: Un contador puede estar vinculado a muchas empresas, pero una empresa solo puede tener un contador activo asociado; el sistema debe impedir multiples asociaciones activas por empresa.
- RF-286: ROOT debe crear o activar contadores, asociarlos a empresas y retirar asociaciones, conservando auditoria de actor, empresa, contador, estado, fecha y motivo cuando aplique.
- RF-287: El contador solo puede consultar informacion de empresas a las que este vinculado activamente; no hereda permisos ROOT ni puede cambiar de empresa por `company_id` arbitrario.
- RF-288: El portal de contador debe permitir consultar reportes de ventas, gastos, compras, nomina, ingresos, egresos, cartera, cuentas por pagar, movimientos contables y resumen fiscal de cada empresa vinculada, segun licencia y permisos aprobados.
- RF-289: El acceso inicial de contadores creados por ROOT debe usar contrasena temporal, vencimiento configurable y cambio obligatorio en primer ingreso antes de acceder al portal.
- RF-290: El acceso inicial del administrador de empresa creado por ROOT debe usar la misma regla de contrasena temporal, vencimiento configurable y cambio obligatorio antes de acceder a modulos empresariales.
- RF-291: Cuando ROOT cree credenciales temporales de contador o administrador empresarial, el sistema debe enviar correo informativo al usuario con instrucciones de ingreso, vigencia y obligatoriedad de cambio de clave.
- RF-292: El modulo de terceros debe seguir siendo la fuente canonica de datos fiscales de clientes/proveedores: responsabilidades fiscales, regimen tributario, municipio, tipo de persona y roles. No se debe crear una configuracion fiscal paralela para proveedores.
- RF-293: El modulo de terceros debe agregar campo `ciiuCode` para registrar actividad economica CIIU del tercero cuando aplique, especialmente proveedores usados en compras, gastos y retenciones.
- RF-294: El sistema debe implementar reglas fiscales/contables versionadas para calcular retenciones sobre compras, gastos, pagos y operaciones aplicables, combinando perfil fiscal de la empresa, perfil fiscal del proveedor/tercero, municipio, CIIU, concepto, base, fecha, regimen y responsabilidades DIAN.
- RF-295: Las reglas de retencion no deben estar hardcodeadas en formularios; deben vivir en catalogos/reglas versionadas trazables a Estatuto Tributario, DUR, PUC, RUT/responsabilidades DIAN y parametros municipales cuando aplique.
- RF-296: La configuracion inicial de empresa debe incluir perfil fiscal/contable para determinar obligaciones: tamano empresarial, grupo contable/NIIF aplicable, regimen tributario, responsabilidades RUT, responsable de IVA, agente retenedor, gran contribuyente, autorretenedor, regimen SIMPLE, municipio ICA y actividades economicas CIIU.
- RF-297: Compras y gastos deben calcular y conservar retenciones aplicables, valor neto a pagar, cuentas sugeridas PUC y evidencia de la regla usada; si falta parametrizacion fiscal o contable obligatoria, deben fallar con error funcional auditable.
- RF-298: Inventario debe enviar notificacion por correo cuando un producto alcance o cruce su umbral de desabastecimiento configurado, evitando correos duplicados por el mismo evento dentro de una ventana de control.
- RF-299: Los reportes avanzados asincronos deben enviar correo cuando queden listos, reutilizando links intermediados de descarga y sin exponer URL directa de storage, bucket, key, token interno o credenciales.
- RF-300: Toda notificacion por correo debe pasar por un puerto de notificaciones desacoplado, con adaptador local y adaptador SMTP/SES o equivalente productivo, auditoria de intento/resultado y sanitizacion de secretos.
- RF-301: El catalogo fiscal debe versionar UVT, conceptos, bases, tarifas, decisiones, fuentes normativas y vigencias; una regla aplicada nunca puede depender de una migracion historica modificada ni de valores hardcodeados en la UI.
- RF-302: El motor debe resolver bases en COP, UVT o valor de IVA, soportar umbral inclusivo/exclusivo, calculo sobre valor total o excedente y seleccionar una sola regla efectiva por tipo de retencion.
...
- RF-317: ROOT debe crear y actualizar una empresa junto con su perfil fiscal desde un unico formulario, sin depender de la empresa activa; el regimen es obligatorio y la persistencia de ambos agregados debe ser transaccional.
- RF-318: El formulario de reglas fiscales debe seleccionar municipio DIVIPOLA, CIIU, regimen, responsabilidad, concepto y tercero desde catalogos o datos empresariales autorizados, conservar opciones sin restriccion y evitar captura libre de codigos o UUID.
- RF-319: Una exencion dirigida a un tercero debe exigir un unico soporte PDF privado de maximo 5 MB, almacenado como archivo empresarial con hash, metadata, aislamiento por empresa y descarga autorizada; la regla solo conserva una referencia interna estable.
- RF-320: ROOT no debe publicar una regla empresarial sin empresa activa ni asociar un tercero o soporte empresarial a una regla nacional global; el formulario debe limpiar condiciones incompatibles al cambiar alcance o decision.
- RF-321: La configuracion tributaria debe exponerse como `Reglas fiscales` y quedar protegida por el permiso empresarial delegable `FISCAL_SETTINGS_MANAGE`; este permiso cubre perfil fiscal, reglas, soportes, emisor, politica, resoluciones y conexion DIAN, pero no reemplaza `FISCAL_DOCUMENTS_ISSUE` para emitir documentos.
- RF-322: Los selectores fiscales con busqueda deben usar un unico combobox con busqueda dentro del desplegable, seleccion controlada y navegacion por teclado; no deben presentar un campo de busqueda separado del selector.
- RF-323: El selector de tercero exento debe cargar proveedores activos de la empresa actual y permitir buscar por nombre, razon social, nombre comercial o identificacion, informando de forma controlada carga, error y ausencia de resultados.
- RF-324: `Configuracion contable` debe pertenecer al grupo de navegacion `Configuracion`, y `Catalogo fiscal` debe renombrarse `Reglas fiscales` en toda la interfaz sin cambiar el modulo de licencia `ACCOUNTING`.
- RF-325: El rol empresarial `OWNER` materializado al crear el administrador inicial debe contener todos los permisos `COMPANY`, incluidos `COMPANY_SETTINGS_MANAGE` y `FISCAL_SETTINGS_MANAGE`; los OWNER existentes deben recibir el nuevo permiso y poder delegarlo mediante roles personalizados sin otorgar alcance global.
- RF-326: `COMPANY_SETTINGS_MANAGE` debe autorizar la actualizacion de datos generales de la empresa activa, mientras `FISCAL_SETTINGS_MANAGE` debe autorizar la actualizacion aislada de su perfil fiscal; ninguna autorizacion empresarial puede operar sobre otra empresa.
- RF-327: Las licencias comerciales deben usar exclusivamente los planes `POS`, `FULL` y `CUSTOM`, donde los dos primeros son presets cerrados y `CUSTOM` permite seleccion granular.
- RF-328: La licencia debe persistir `enabledModules` para agrupacion/navegacion y `enabledFeatures` para distinguir capacidades basicas, avanzadas y comerciales dentro de un mismo modulo.
- RF-329: El preset `POS` debe habilitar empresa basica, branding basico, clientes, productos/servicios, inventario basico, ventas POS, facturacion electronica, registro de ventas, configuracion y documentos fiscales basicos, reportes sincronicos, catalogos operativos, logs basicos, usuarios/RBAC basico, PIN operacional y contabilidad interna necesaria para cerrar ventas.
- RF-330: El preset `POS` no debe habilitar proveedores, compras, gastos, deudores, contabilidad visible avanzada, nomina, reportes asincronos, reglas fiscales avanzadas, auditoria avanzada, portal contador ni capacidades de personalizacion.
- RF-331: El preset `FULL` debe habilitar todos los modulos y capacidades estandar implementadas, sin incluir automaticamente portal contador ni servicios de personalizacion aun no implementados.
- RF-332: El plan `CUSTOM` debe permitir que ROOT seleccione modulos y capacidades compatibles, incluyendo marcadores comerciales de reglas, reportes, flujos, integraciones y soporte prioritario.
- RF-333: Los planes `POS` y `FULL` deben normalizarse en backend, ignorando selecciones inconsistentes enviadas por el cliente; `CUSTOM` debe validar que cada capacidad seleccionada corresponda a un modulo habilitado o sea una capacidad comercial permitida.
...
- RF-343: La creacion de una empresa por ROOT debe capturar los datos del RUT y las condiciones complementarias necesarias para determinar la obligacion de facturar; el usuario no puede seleccionar directamente el resultado de la clasificacion.
- RF-344: La clasificacion de facturacion debe ser calculada en backend y usar los estados `OBLIGATED`, `NOT_OBLIGATED_VERIFIED`, `VOLUNTARY_ELECTRONIC`, `REVIEW_REQUIRED` y `TRANSITION_TO_OBLIGATED`.
- RF-345: Cualquier causal verificada de obligacion debe prevalecer sobre una causal de excepcion. Datos ausentes, contradictorios, vencidos o sin evidencia deben producir `REVIEW_REQUIRED`, nunca habilitar venta no fiscal.
- RF-346: La responsabilidad RUT `52 - Facturador electronico`, el regimen SIMPLE, la responsabilidad de IVA, la responsabilidad de impuesto nacional al consumo y las actividades expresamente obligadas deben impedir `NON_FISCAL_SALE`.
- RF-347: Las responsabilidades `49 - No responsable de IVA`, `50 - No responsable de consumo`, `53 - Persona juridica no responsable de IVA` o `57 - Persona juridica no responsable de consumo` no deben determinar por si solas que un sujeto no esta obligado a facturar.
- RF-348: La excepcion de persona natural no responsable de IVA debe exigir conjuntamente los umbrales de ingresos, establecimientos, intangibles, condicion aduanera, contratos y operaciones financieras previstos en el articulo 437 del Estatuto Tributario, ademas de no presentar otra causal de obligacion.
- RF-349: Las excepciones para persona natural no responsable de consumo, venta exclusiva de bienes excluidos o servicios no gravados y entidades u operaciones especiales deben evaluarse con su alcance, condiciones y evidencia propias, sin convertir una excepcion por operacion en una excepcion general de empresa.
- RF-350: Los umbrales deben expresarse en UVT, resolverse por fecha desde el catalogo fiscal versionado y conservar el operador normativo aplicable; para 2026 el valor de referencia de 3.500 UVT es COP 183.309.000, sin hardcodearlo en dominio o UI.
- RF-351: El RUT PDF debe almacenarse como evidencia privada con empresa, fecha de generacion, hash, metadata, version y acceso auditado. La primera version no requiere OCR; ROOT registra datos mediante controles de catalogo y el archivo sirve como soporte de revision.
- RF-352: Cada cambio de RUT, persona, regimen, responsabilidades, CIIU, establecimientos o condiciones cuantitativas debe crear un nuevo snapshot, recalcular la clasificacion y conservar historial, motivos, version normativa, actor y fecha.
- RF-353: Solo `NOT_OBLIGATED_VERIFIED` vigente puede usar `NON_FISCAL_SALE`. Los demas estados deben bloquear ese tipo tanto en politica como en override y en confirmacion de venta; el PIN operacional nunca puede reemplazar esta validacion.
- RF-354: El comprobante de una venta no fiscal debe identificarse expresamente como comprobante interno, indicar que no es factura ni documento equivalente y que no sirve como soporte fiscal, sin CUFE, CUDE ni QR DIAN.
- RF-355: El sistema debe reevaluar condiciones dinamicas con datos operacionales disponibles y exigir certificacion periodica para datos no verificables automaticamente. Al detectar una causal de obligacion debe impedir nuevas ventas no fiscales, registrar la transicion y orientar la habilitacion DIAN sin alterar ventas historicas.
- RF-356: Los indicadores nacionales del perfil fiscal de empresa deben derivarse en backend desde responsabilidades RUT controladas: retencion en la fuente `07`, gran contribuyente `13`, autorretencion `15` o `59`, agente de ReteIVA `23`, SIMPLE `47` y responsable de IVA `48`; el cliente no puede sobrescribirlos mediante booleanos contradictorios.
- RF-357: La calidad de agente de ReteICA debe permanecer como una designacion municipal explicita, asociada al municipio DIVIPOLA de la empresa, porque no se deduce de las responsabilidades nacionales del RUT.
- RF-358: El backend debe rechazar responsabilidades incompatibles, como `48` con `49`, y la responsabilidad excluyente `R-99-PN` combinada con cualquier otra; el catalogo debe ofrecer los codigos nacionales usados por el motor.
- RF-359: Las excepciones especiales declaradas en el cuestionario no deben producir `NOT_OBLIGATED_VERIFIED` sin una verificacion especifica de sujeto, operacion, alcance y soporte; mientras esa verificacion no exista deben producir `REVIEW_REQUIRED`.
- RF-360: La carga de evidencia debe admitir en base de datos las categorias `RUT_EVIDENCE` y `FISCAL_RULE_EVIDENCE`, conservando la validacion de PDF, tamano, firma y aislamiento empresarial.
- RF-361: El BFF debe reenviar solicitudes `multipart/form-data` como bytes opacos, conservando el `Content-Type` con su `boundary`; no debe resolver ni consumir las partes antes de entregarlas al microservicio propietario.

## Requisitos de culminacion del motor fiscal colombiano

- RF-362: Toda fuente normativa fiscal debe registrarse con autoridad emisora, tipo y numero de acto, articulos aplicables, URL oficial HTTPS, fecha de publicacion, estado juridico y eventos temporales `PUBLISHED`, `EFFECTIVE`, `SUSPENDED`, `REACTIVATED` o `REPEALED`; una regla no puede considerarse vigente solo porque su fecha inicial ya transcurrio.
- RF-363: El calculo debe seleccionar el conjunto normativo aplicable en la fecha de pago o abono en cuenta y conservarlo en un snapshot inmutable; una suspension, reactivacion o derogatoria posterior no puede alterar calculos confirmados.
- RF-364: La vigencia de los articulos 2 a 8 del Decreto 572 de 2025 debe mantenerse en estado pendiente de verificacion oficial para fechas posteriores a la suspension informada por la DIAN el 8 de mayo de 2026; ninguna version posterior puede publicarse sin registrar la providencia ejecutoriada o comunicacion oficial que determine su fecha efectiva.
- RF-365: La calidad del pagador como agente de retefuente, ReteIVA, ReteICA o autorretenedor debe resolverse desde un perfil empresarial temporal, responsabilidades RUT, designaciones expresas y reglas legales; los indicadores del navegador no son autoritativos.
- RF-366: El perfil fiscal temporal del beneficiario debe distinguir persona natural o juridica, residente o no residente, declarante, regimen ordinario/SIMPLE/especial, responsabilidades RUT, gran contribuyente, agente de ReteIVA, autorretenedor y alcance de su designacion, municipio, CIIU y evidencias vigentes.
- RF-367: Compras, gastos y pagos deben clasificar fiscalmente cada linea; un documento puede contener multiples conceptos, bases, tarifas y decisiones, y el total por tipo de retencion debe ser la suma de sus lineas y agregaciones aplicables.
- RF-368: Cada regla debe declarar el momento de causacion (`PAYMENT`, `ACCRUAL` o el evento legal correspondiente), la base (`TAXABLE_AMOUNT`, `VAT_AMOUNT`, `AIU`, `GROSS_PAYMENT`, `COMPANY_INCOME` u otra cerrada) y el tratamiento del umbral (`FULL_AMOUNT`, `EXCESS` o `BRACKETED`).
- RF-369: El motor debe soportar umbrales por operacion y acumulaciones normativas por beneficiario, comprador, concepto, contrato, dia, mes o ano; los reintentos y documentos anulados no pueden duplicar acumulados.
- RF-370: El catalogo nacional de retefuente debe cubrir al menos compras generales y especiales, servicios, honorarios, comisiones, arrendamientos, transporte, contratos, rendimientos, pagos laborales/no laborales, bienes inmuebles, pagos al exterior y otros ingresos, cada uno con sujeto, base, tarifa, excepciones y fuente vigente.
- RF-371: ReteIVA debe evaluar la calidad legal del comprador y vendedor, IVA efectivamente causado, operaciones entre agentes, cuantias minimas, residencia, servicios desde el exterior y tarifas generales o especiales; ser responsable de IVA no equivale por si solo a ser agente de ReteIVA.
- RF-372: ReteICA debe usar un paquete territorial publicado para el municipio donde se causa la actividad conforme a territorialidad, no asumir el domicilio del proveedor; cada paquete debe versionar agentes, sujetos, actividades CIIU, conceptos, bases, tarifas, excepciones, calendario y fuente municipal.
- RF-373: La calidad de autorretenedor del proveedor no debe convertirse en una exclusion universal de retefuente; cada regla debe expresar el impuesto, concepto y alcance en que procede o no la retencion del comprador. La autorretencion propia se calcula sobre ingresos y CIIU de la empresa segun la version vigente.
- RF-374: El regimen SIMPLE debe evaluarse por impuesto y operacion: la exclusion de retencion a titulo de renta no puede reutilizarse automaticamente para IVA, ICA, pagos laborales u otros tratamientos con norma propia.
- RF-375: Exenciones, no sujeciones y tarifas especiales deben tener precedencia deterministica, soporte, periodo, alcance y autoridad; un soporte privado no puede modificar una regla nacional, y una excepcion vencida debe dejar de aplicar automaticamente.
- RF-376: Cada resultado debe explicar pagador, beneficiario, concepto, territorio, base, acumulado, umbral, tarifa, decision, fuente, version, estado juridico y fecha de consulta; datos insuficientes o reglas incompatibles deben producir `BLOCKED`, nunca tarifa cero asumida.
- RF-377: El sistema debe generar auxiliares por periodo para Formulario 350, declaraciones territoriales, retenciones por pagar, retenciones a favor y certificados por tercero, permitiendo conciliacion contra snapshots sin afirmar que presenta automaticamente declaraciones ante la DIAN o municipios.
- RF-378: Notas credito, anulaciones, devoluciones y retenciones practicadas en exceso deben generar reversos vinculados al snapshot original, sin editarlo ni borrar su trazabilidad.
- RF-379: La configuracion contable debe identificar el marco colombiano aplicable del Decreto 2420 de 2015: Grupo 1, Grupo 2 o Grupo 3, con vigencia, evidencia y politicas contables propias.
- RF-380: El plan de cuentas debe ser configurable por empresa. Los codigos historicos del Decreto 2650 de 1993 pueden usarse como plantilla y lenguaje operativo, pero no deben presentarse como un PUC NIIF universal ni impedir planes sectoriales o cuentas auxiliares propias.
- RF-381: Las reglas de contabilizacion deben separar naturaleza financiera, tratamiento tributario y mapeo de presentacion; deben soportar conciliacion contable-fiscal sin permitir que una tarifa fiscal determine por si sola reconocimiento o medicion NIIF.
- RF-382: La confirmacion de una operacion debe ser atomica en sus efectos funcionales: calculo, snapshots, asiento, cuenta por pagar y acumulados quedan completos o la operacion permanece pendiente; las llamadas entre servicios requieren idempotencia y compensacion observable.
...
## Requisitos de despliegue AWS Free Preview

- RF-391: La cuenta AWS `883425315805` debe disponer de un entorno `free-preview` independiente, temporal y no productivo; no reemplaza la arquitectura AWS productiva ni autoriza declarar la aplicacion lista para produccion.
- RF-392: Antes de `plan` o despliegue, el proceso debe validar mediante STS que opera exclusivamente en la cuenta permitida y consultar que el plan AWS sea `FREE` y `ACTIVE`, con creditos y fecha de expiracion suficientes segun umbrales configurados.
- RF-393: El entorno `free-preview` debe usar una lista cerrada de recursos y rechazar NAT Gateway, ECS Fargate, RDS/Aurora, RDS Proxy, balanceadores, Route 53, WAF, Secrets Manager, claves KMS administradas por el cliente y observabilidad administrada.
- RF-394: La topologia de menor costo debe ejecutar frontend, BFF, microservicios y PostgreSQL en contenedores sobre una unica instancia EC2 temporal con un unico volumen EBS `gp3` de maximo 30 GB; ningun servicio interno ni la base de datos puede exponerse directamente.
- RF-395: El navegador debe entrar por HTTPS mediante una distribucion CloudFront exclusiva del proyecto y alcanzar solo el proxy web/BFF; el acceso administrativo debe usar Systems Manager Session Manager, sin SSH publico.
- RF-396: Los secretos deben mantenerse fuera del repositorio mediante Parameter Store Standard `SecureString` y llaves administradas por AWS; los artefactos de despliegue deben permanecer en almacenamiento privado, con cifrado, version minima y expiracion automatica.
- RF-397: La instancia no debe iniciar automaticamente, debe detenerse como maximo cuatro horas despues de cada arranque y debe contar con una detencion de respaldo; el presupuesto y las alertas complementan esta barrera, pero no se consideran un limite de gasto en tiempo real.
- RF-398: El entorno debe tener inventario, etiquetas, logs locales rotados, healthchecks, respaldo y destruccion reproducible. Los datos requeridos deben exportarse antes de agotar creditos o alcanzar la fecha de expiracion del plan.
- RF-399: Antes de crear recursos AWS, las imagenes de produccion deben superar pruebas funcionales y una prueba local de memoria que demuestre operacion estable dentro del presupuesto util de una instancia de 8 GiB; si falla, el despliegue queda bloqueado y no escala automaticamente a una instancia mayor.
- RF-400: `terraform apply`, publicacion de artefactos, arranque, destruccion y cualquier cambio IAM o de facturacion requieren confirmacion explicita separada. El proceso nunca debe actualizar el plan AWS, crear Organizations/Control Tower ni reutilizar recursos ajenos existentes.
- RF-401: El dominio publico del preview debe ser `app.nexofiscal.online`, administrado en Hostinger. La activacion debe realizarse en dos pasos: validacion DNS del certificado ACM en `us-east-1` y, solo cuando el certificado este `ISSUED`, CNAME `app` hacia la distribucion CloudFront exclusiva.
